...
exports.Bot.prototype.addNetwork = function(n, conf) {
  ...
  this.on("PRIVMSG", function(serv, m) {      
    ...
    if(msg[0] == '\001') {
      ...
      if(i != null && i.index == 0) {
        m['command'] = msg.slice(1).split(' ', 1)[0];
        m['commandargs'] = msg.slice(m['command'].length + 1).replace(/^\s+/, '');
        cmd.emit(m['command'], serv, m);
      }
    }
  });
